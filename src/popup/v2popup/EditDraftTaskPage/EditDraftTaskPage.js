...
import {removeDraftedTask} from "./services";

export default function EditDraftTaskPage({draft, ...props}) {
  console.log("draft", draft)
  const [taskDetails, setTaskDetails] = useState(draft.taskDetails || "");  // Task Title
  const isTaskDrafted = useSelector(selectIsTaskDrafted);
  const [date, setDate] = useState("");
  const [time, setTime] = useState(draft.time || "00:00");
  const [taskType, setTaskType] = useState(draft.actionType || TASK_TYPE.REMINDER);
  const [priority, setPriority] = useState(draft.priority || PRIORITIES.none);
  ...
  useEffect(() => {
    if(draft.description) {
      fetch(draft.description).then(res => {
